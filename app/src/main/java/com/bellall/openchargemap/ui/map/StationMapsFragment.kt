...
import com.bellall.openchargemap.databinding.FragmentStationMapsBinding
...
class StationMapsFragment : Fragment() {
    ...
    private fun initObserver() {
        with(sharedViewModel) {
            ...
            hasDataLoadedLiveData().observe(viewLifecycleOwner) {
                if (it) {
                    Toast.makeText(requireContext(), "Data loaded", Toast.LENGTH_SHORT).show()
                }
            }

            getstationMutableList().observe(viewLifecycleOwner, observeStationListData)
